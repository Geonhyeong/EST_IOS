...
final class ConnectionManager: NSObject, GCDAsyncUdpSocketDelegate{
    ...
    fileprivate var socket_reg:GCDAsyncUdpSocket!
    ...
    var ID:String?
    {
        get {
            if (_id == nil){
                _id = UserDefaults.standard.string(forKey: ID_KEY)
            }
            return _id
        }
        set{
            _id = newValue
            UserDefaults.standard.set(newValue, forKey: ID_KEY)
            UserDefaults.standard.synchronize()
        }
    }
// MARK: 
//    ======================================================================================
    override init()
    {
        ...
        socket_reg = GCDAsyncUdpSocket(delegate: self, delegateQueue: DispatchQueue.main)
    ...
    convenience init(dataObservable:Observable<Data>)
    {
        ...
        dataObservable.subscribe(onNext: { [weak self] data in
            ...
            let proto = Header(command: Header.VoiceCommand,
                               idlength: (self?.ID?.count)!,
                               id: (self?.ID)!,
                               datalength: data.count).toByteArray()
...
extension ConnectionManager {
    ...
    func connect(receiveBlock:@escaping (Data)->()) throws
    {
        if let reachabilityStatus = appDelegate.reachability?.currentReachabilityStatus, case .reachableViaWiFi = reachabilityStatus{
            ...
            if let rmPort = remotePort, let inPort = incommingPort, let addr = remoteAddress{
                ...
                let register = Data(bytes: proto)
                socket_reg.send(register, withTimeout: 0, tag: 0)
                try socket_reg.beginReceiving()
    ...
    func disconnect()
    {
        let proto = Header(command: Header.CancelCommand,
                           idlength: (ID?.count)!, id: ID!,
                           datalength: 0).toByteArray()
        let cancel = Data(bytes:proto)
        socket_reg.send(cancel, withTimeout: 0, tag: 0)
        socket_reg.close()
        socket.close()
    }
